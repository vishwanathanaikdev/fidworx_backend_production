...
// ✅ Fetch Co-Working Space by ID
async function getCoWorkingSpaceById({ reqApiKey, Id }) {
  try {
    if (xkey !== reqApiKey) {
    ...
    const data = await CoWorkingSpaceModel.findById(Id).populate("assigned_agent", "fullName name firstName lastName userId").lean();
    ...
    console.error("Error fetching co-working space by ID:", error);
...
export const GET = async (req, res) => {
  try {
    ...
    const { data, error } = await getCoWorkingSpaceById({
      reqApiKey,
      Id
    });
    ...
    return NextResponse.json(
      { success: true, message: "Co-Working Space fetched successfully", data },
      { status: 200 }
    );
  } catch (error) {
