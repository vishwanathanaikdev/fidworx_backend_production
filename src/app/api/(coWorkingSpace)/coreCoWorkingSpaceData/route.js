...
// ✅ Fetch Co-Working Spaces Function
async function getCoWorkingSpaces({ reqApiKey, page, size, search }) {
  try {
    if (xkey !== reqApiKey) {
    ...
    const data = await CoWorkingSpaceModel.find(filter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean();

    const total = await CoWorkingSpaceModel.countDocuments(filter);
...
export const GET = async (req, res) => {
  try {
    ...
    const { data, total, error } = await getCoWorkingSpaces({
      reqApiKey,
      page,
      size,
      search
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Co-working space data fetched successfully",
        data,
        total
      },
      { status: 200 }
