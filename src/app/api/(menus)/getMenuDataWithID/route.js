// app/api/menus/getById/route.js
...
async function getMenuDataWithID({ reqApiKey, _id, gxkey }) {
    try {
        ...
        const data = await MenusModel.findById(_id).lean();
        if (!data) {
...
export async function GET(req) {
    ...
    const reqApiKey =await headerList.get("x-api-key");
    const gxkey = req.nextUrl.searchParams.get('authkey');
    const _id = req.nextUrl.searchParams.get('MenuID');

    try {
        const result = await getMenuDataWithID({ reqApiKey, _id, gxkey });
        if (result.success) {
