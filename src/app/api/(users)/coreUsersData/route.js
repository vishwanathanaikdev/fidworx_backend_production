...
async function getUsers({ reqApiKey, page, size, search }) {
  try {
    if (xkey !== reqApiKey) {
    ...
    // Fetch users in descending order by creation date
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Invalid page or size parameter. Both must be positive integers greater than zero.",
        },
        { status: 400 }
      );
    }

    const { data, total, error } = await getUsers({
      reqApiKey,
      page,
      size,
      search,
